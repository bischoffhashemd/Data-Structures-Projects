package kindergarten;

/**
 * This class represents a Classroom, with:
 * - an SNode instance variable for students in line,
 * - an SNode instance variable for musical chairs, pointing to the last student in the list,
 * - a boolean array for seating availability (eg. can a student sit in a given seat), and
 * - a Student array parallel to seatingAvailability to show students filed into seats 
 * --- (more formally, seatingAvailability[i][j] also refers to the same seat in studentsSitting[i][j])
 * 
 * @author Ethan Chou
 * @author Kal Pandit
 * @author Maksims Kurjanovics Kravcenko
 */
public class Classroom {
    private SNode studentsInLine;             // when students are in line: references the FIRST student in the LL
    private SNode musicalChairs;              // when students are in musical chairs: references the LAST student in the CLL
    private boolean[][] seatingAvailability;  // represents the classroom seats that are available to students
    private Student[][] studentsSitting;      // when students are sitting in the classroom: contains the students

    /**
     * Constructor for classrooms. Do not edit.
     * @param l passes in students in line
     * @param m passes in musical chairs
     * @param a passes in availability
     * @param s passes in students sitting
     */
    public Classroom ( SNode l, SNode m, boolean[][] a, Student[][] s ) {
		studentsInLine      = l;
        musicalChairs       = m;
		seatingAvailability = a;
        studentsSitting     = s;
	}
    /**
     * Default constructor starts an empty classroom. Do not edit.
     */
    public Classroom() {
        this(null, null, null, null);
    }

    /**
     * This method simulates students coming into the classroom and standing in line.
     * 
     * Reads students from input file and inserts these students in alphabetical 
     * order to studentsInLine singly linked list.
     * 
     * Input file has:
     * 1) one line containing an integer representing the number of students in the file, say x
     * 2) x lines containing one student per line. Each line has the following student 
     * information separated by spaces: FirstName LastName Height
     * 
     * @param filename the student information input file
     */
    public void makeClassroom ( String filename ) {

        // WRITE YOUR CODE HERE
        StdIn.setFile(filename);
        int numOfStudents = StdIn.readInt();

       while (!StdIn.isEmpty())
        {
            String firstName = StdIn.readString();
            String lastName = StdIn.readString();
            int height = StdIn.readInt();
            
            Student student = new Student( firstName, lastName, height);

            if (studentsInLine == null) //this student is first in line
            {
                studentsInLine = new SNode(student, null);
            }

            else // there are other students in line
            { 
                SNode nextStudent = new SNode(student, null);
                SNode ptr = studentsInLine;

                //creating previous node so that previous.next points to the head (studentsInLine)
                SNode dummy = new SNode(null,null);
                dummy.setNext(studentsInLine);
                SNode previous = dummy;
                int nodeNumber = 1;

                //finding where to place student in line
                while (ptr != null)
                {
                    int comesFirst = student.compareNameTo(ptr.getStudent());

                    // new node comes after pointer (we have not reached the position to put it yet)
                    if (comesFirst > 0 && ptr.getNext() != null)
                    {
                        ptr = ptr.getNext();
                        previous = previous.getNext();
                        nodeNumber += 1;
                    }

                    else if (comesFirst > 0 && ptr.getNext() == null)
                    {
                        ptr.setNext(nextStudent);
                        break;
                    }

                    else // comesFirst < 0
                    {
                        nextStudent.setNext(ptr);
                        previous.setNext(nextStudent);

                        if (nodeNumber == 1)
                        {
                            studentsInLine = nextStudent;
                        } 
                        break;
                    }
                }
              
                //SNode next = studentsInLine;
                //studentsInLine = new SNode(student, next);
            }
        }
    }

    /**
     * 
     * This method creates and initializes the seatingAvailability (2D array) of 
     * available seats inside the classroom. Imagine that unavailable seats are broken and cannot be used.
     * 
     * Reads seating chart input file with the format:
     * An integer representing the number of rows in the classroom, say r
     * An integer representing the number of columns in the classroom, say c
     * Number of r lines, each containing c true or false values (true denotes an available seat)
     *  
     * This method also creates the studentsSitting array with the same number of
     * rows and columns as the seatingAvailability array
     * 
     * This method does not seat students on the seats.
     * 
     * @param seatingChart the seating chart input file
     */
    public void setupSeats(String seatingChart) {
        // WRITE YOUR CODE HERE
         StdIn.setFile(seatingChart);
         int rowsInClassroom = StdIn.readInt();
         int columnsInClassroom = StdIn.readInt();
         
         // setting dimensions of arrays
         studentsSitting = new Student[rowsInClassroom][columnsInClassroom];
         seatingAvailability = new boolean[rowsInClassroom][columnsInClassroom];

         for (int r = 0; r < rowsInClassroom; r++)
         {
            for (int c = 0; c < columnsInClassroom; c++)
            {
                seatingAvailability[r][c] = StdIn.readBoolean();
            }
        }
    }

    /**
     * 
     * This method simulates students taking their seats in the classroom.
     * 
     * 1. seats any remaining students from the musicalChairs starting from the front of the list
     * 2. starting from the front of the studentsInLine singly linked list
     * 3. removes one student at a time from the list and inserts them into studentsSitting according to
     *    seatingAvailability
     * 
     * studentsInLine will then be empty
     */
    public void seatStudents () {

	    // WRITE YOUR CODE HERE

        outerloop: 
        for (int r = 0; r < seatingAvailability.length; r++)
        {
            for (int c = 0; c < seatingAvailability[r].length; c++)
            {
                if (seatingAvailability[r][c] == true)
                {
                    // seat winning student
                    if (musicalChairs != null)
                    { 
                        studentsSitting[r][c] = musicalChairs.getStudent();
                        musicalChairs = null;
                    }

                    // use temp to remove links between nodes as they're moved from line to classroom
                   else // there is no musical chairs winner or winner is already seated
                   {
                        if (studentsInLine != null) //delete this condition if code is not working
                        {
                            //SNode temp = new SNode(null,null);
                            //temp.setNext(studentsInLine);
    
                            studentsSitting[r][c] = studentsInLine.getStudent();
                            studentsInLine = studentsInLine.getNext();
                            //temp.setNext(null);   
                        }                        

                        if (studentsInLine == null) 
                        {
                            break outerloop;
                        }
                    } 

                }  
            }
        }
    }
    

    /**
     * Traverses studentsSitting row-wise (starting at row 0) removing a seated
     * student and adding that student to the end of the musicalChairs list.
     * 
     * row-wise: starts at index [0][0] traverses the entire first row and then moves
     * into second row.
     */
    public void insertMusicalChairs () {
        
        // WRITE YOUR CODE HERE

        SNode first = new SNode(null,null);
        for (int r = 0; r < studentsSitting.length; r++)
        {
            for (int c = 0; c < studentsSitting[r].length; c++)
            {
                if (studentsSitting[r][c] != null)
                {
                    if (musicalChairs == null)
                    {
                        Student student = studentsSitting[r][c];
                        musicalChairs = new SNode(student, null); 
                        musicalChairs.setNext(musicalChairs);
                        first = musicalChairs;
                        studentsSitting[r][c] = null;
                    }
                    
                    else
                    {
                        Student student = studentsSitting[r][c];
                        SNode nextStudent = new SNode(student, null);
                        musicalChairs.setNext(nextStudent);
                        musicalChairs = nextStudent;
                        nextStudent.setNext(first);
                        studentsSitting[r][c] = null;
                    }   
                }
            }
        }

     }

    /**
     * 
     * This method repeatedly removes students from the musicalChairs until there is only one
     * student (the winner).
     * 
     * Choose a student to be elimnated from the musicalChairs using StdRandom.uniform(int b),
     * where b is the number of students in the musicalChairs. 0 is the first student in the 
     * list, b-1 is the last.
     * 
     * Removes eliminated student from the list and inserts students back in studentsInLine 
     * in ascending height order (shortest to tallest).
     * 
     * The last line of this method calls the seatStudents() method so that students can be seated.
     */
    public void playMusicalChairs() {

        // WRITE YOUR CODE HERE

        //setup counter and pointer
        int studentsInGameCount = 0;
        if (musicalChairs != null)
        {
            studentsInGameCount = 1;
        }
        SNode ptr = musicalChairs.getNext();
     
        // count number of students in musical chairs
        while (ptr != musicalChairs && studentsInGameCount != 0)
        {
            studentsInGameCount +=1; 
            ptr = ptr.getNext();
        }
        
        // if there are at least 2 students (meaning students need to be eliminated from musical chairs)
        while (studentsInGameCount > 1) 
        {
            int n = StdRandom.uniform(studentsInGameCount); 

            // musical chairs pointers
            SNode mcPrev = musicalChairs; 
            SNode mcPtr = musicalChairs.getNext();

            //int count = 0; 
            // finding node to remove from musicalChairs and adding it to line
           // while (count < studentsInGameCount)  
            for (int count = 0; count <= n; count++) 
            {
                // adding FIRST student to line
                if (count == n && studentsInLine == null) 
                {
                    SNode firstStudent = new SNode (mcPtr.getStudent(), null);
                    studentsInLine = firstStudent;
                    if (mcPtr == musicalChairs) 
                    {
                        musicalChairs = mcPrev;
                    }
                    mcPrev.setNext(mcPtr.getNext());
                }

                // adding student who's NOT FIRST to line 
                else if (count == n && studentsInLine != null)
                {
                    // line pointers
                    SNode linePtr = studentsInLine;
                    SNode linePrev = new SNode(null, studentsInLine);

                    Student studentOfEliminatedNode = mcPtr.getStudent(); //CHANGE BACK IF THERE ARE ISSUES
                    SNode addingStudent = new SNode (studentOfEliminatedNode, null); //save student being removed to an array
                    
                    if (mcPtr == musicalChairs) 
                    {
                        musicalChairs = mcPrev;
                    }
                    mcPrev.setNext(mcPtr.getNext());
    
                    //finding where to place student in line
                    whileLoop:
                    while (linePtr != null)
                    {   
                        // creating variables to store height of ptr and of the student being added to the line
                        int addingStudentHeight = addingStudent.getStudent().getHeight();
                        int linePtrHeight = linePtr.getStudent().getHeight();

                        // new node comes after pointer (we have not reached the position to put it yet)
                        if (addingStudentHeight > linePtrHeight && linePtr.getNext() != null)
                        {
                            // student is taller than ptr student, increment current pointer and previous pointer
                            linePtr = linePtr.getNext();
                            linePrev = linePrev.getNext();
                        }
    
                        else if (addingStudentHeight > linePtrHeight && linePtr.getNext() == null)
                        {
                            // add student to end of line and break out of while loop
                            linePtr.setNext(addingStudent);
                            break whileLoop; //DELETE IF CAUSES ISSUES
                        }
    
                        else // addingStudentHeight <= ptrHeight
                        {
                            addingStudent.setNext(linePtr);
                            linePrev.setNext(addingStudent);
    
                            if (linePrev.getStudent() == null) 
                            {
                                studentsInLine = addingStudent;
                            } 
                            break whileLoop;
                        }
                    } 
                }

                // count != n, didn't find target yet
                else 
                {
                    mcPtr = mcPtr.getNext();
                    mcPrev = mcPrev.getNext();
                    //count +=1;
                }
            }
            //n -= 1;   
            studentsInGameCount -= 1;
        }
        // call method to seat students
        seatStudents();
    } 

    /**
     * Insert a student to wherever the students are at (ie. whatever activity is not empty)
     * Note: adds to the end of either linked list or the next available empty seat
     * @param firstName the first name
     * @param lastName the last name
     * @param height the height of the student
     */
    public void addLateStudent ( String firstName, String lastName, int height ) {
        
        // WRITE YOUR CODE HERE

        // create student object and node
        Student lateStudent = new Student(firstName, lastName, height);
        SNode lateStudentNode = new SNode (lateStudent, null);

        // if students are in line
        if (studentsInLine != null)
        {
            SNode ptr = studentsInLine;

            whileLoop:
            while (ptr != null)
            {
                if (ptr.getNext() == null)
                {
                    ptr.setNext(lateStudentNode);
                    break whileLoop;

                }
                else 
                {
                    ptr = ptr.getNext();
                }
            }    
        } 

        // if students are playing musical chairs
        else if (musicalChairs != null)
        {
            lateStudentNode.setNext(musicalChairs.getNext());
            musicalChairs.setNext(lateStudentNode);
            musicalChairs = lateStudentNode;
        }

        // if student is sitting in classroom
        else if (studentsSitting != null)
        {
            outerForLoop:
            for (int r = 0; r < studentsSitting.length; r++)
            {
                for (int c = 0; c < studentsSitting[r].length; c++)
                {
                    if (seatingAvailability[r][c] == true && studentsSitting[r][c] == null)
                    {
                        studentsSitting[r][c] = lateStudentNode.getStudent();
                        break outerForLoop;
                    }
                }
            }
        }
    }

    /**
     * A student decides to leave early
     * This method deletes an early-leaving student from wherever the students 
     * are at (ie. whatever activity is not empty)
     * 
     * Assume the student's name is unique
     * 
     * @param firstName the student's first name
     * @param lastName the student's last name
     */
    public void deleteLeavingStudent ( String firstName, String lastName ) {

        // WRITE YOUR CODE HERE
        String fullName = firstName + " " + lastName;

        // if student is in line
        if (studentsInLine != null)
        {
            SNode ptr = studentsInLine;
            SNode prev = new SNode (null, studentsInLine);

            whileLoop:
            while (ptr != null)
            {
                String currentStudentFullName = ptr.getStudent().getFullName();
                if (fullName.equals(currentStudentFullName))
                {
                    if (ptr == studentsInLine)
                    {
                        studentsInLine = ptr.getNext();
                    }

                    else
                    {
                        prev.setNext(ptr.getNext());
                    }
                    break whileLoop;
                }

                //haven't found student in line yet
                else
                {
                    ptr = ptr.getNext();
                    prev = prev.getNext();
                }
            }
            
        }

        // if student is in musical chairs
        else if (musicalChairs != null)
        {
            SNode ptr = musicalChairs.getNext();
            SNode prev = musicalChairs;

            //count students in musical chairs
            int studentCount = 0;
            SNode ptrForCounting = musicalChairs.getNext();
            while (ptrForCounting != musicalChairs)
            {
                studentCount += 1;
                ptrForCounting = ptrForCounting.getNext();
            }

            whileLoop:
            while (studentCount >= 0)
            {
                String currentStudentFullName = ptr.getStudent().getFullName();
                if (currentStudentFullName.equals(fullName))
                {
                    // FIX if causing issues
                    if (musicalChairs.getNext() == musicalChairs)
                    {
                        musicalChairs = null;
                    }

                    else if (ptr == musicalChairs)
                    {
                        musicalChairs = prev;
                    }
                    prev.setNext(ptr.getNext());
                    
                    break whileLoop;
                }

                else //student leaving not found yet;
                {
                    prev = prev.getNext();
                    ptr = ptr.getNext();
                    studentCount -= 1;
                }
            } 
        }

        // if student is sitting in classroom
        else if (studentsSitting != null)
        {
            for (int r = 0; r < studentsSitting.length; r++)
            {
                for (int c = 0; c < studentsSitting[r].length; c++)
                {
                    if (studentsSitting[r][c] != null)
                    {
                        String currentStudentFullName = studentsSitting[r][c].getFullName();
                        if (currentStudentFullName.equals(fullName))
                        {
                            studentsSitting[r][c] = null;
                        }
                    }
                }
            }
        }

    }

    /**
     * Used by driver to display students in line
     * DO NOT edit.
     */
    public void printStudentsInLine () {

        //Print studentsInLine
        StdOut.println ( "Students in Line:" );
        if ( studentsInLine == null ) { StdOut.println("EMPTY"); }

        for ( SNode ptr = studentsInLine; ptr != null; ptr = ptr.getNext() ) {
            StdOut.print ( ptr.getStudent().print() );
            if ( ptr.getNext() != null ) { StdOut.print ( " -> " ); }
        }
        StdOut.println();
        StdOut.println();
    }

    /**
     * Prints the seated students; can use this method to debug.
     * DO NOT edit.
     */
    public void printSeatedStudents () {

        StdOut.println("Sitting Students:");

        if ( studentsSitting != null ) {
        
            for ( int i = 0; i < studentsSitting.length; i++ ) {
                for ( int j = 0; j < studentsSitting[i].length; j++ ) {

                    String stringToPrint = "";
                    if ( studentsSitting[i][j] == null ) {

                        if (seatingAvailability[i][j] == false) {stringToPrint = "X";}
                        else { stringToPrint = "EMPTY"; }

                    } else { stringToPrint = studentsSitting[i][j].print();}

                    StdOut.print ( stringToPrint );
                    
                    for ( int o = 0; o < (10 - stringToPrint.length()); o++ ) {
                        StdOut.print (" ");
                    }
                }
                StdOut.println();
            }
        } else {
            StdOut.println("EMPTY");
        }
        StdOut.println();
    }

    /**
     * Prints the musical chairs; can use this method to debug.
     * DO NOT edit.
     */
    public void printMusicalChairs () {
        StdOut.println ( "Students in Musical Chairs:" );

        if ( musicalChairs == null ) {
            StdOut.println("EMPTY");
            StdOut.println();
            return;
        }
        SNode ptr;
        for ( ptr = musicalChairs.getNext(); ptr != musicalChairs; ptr = ptr.getNext() ) {
            StdOut.print(ptr.getStudent().print() + " -> ");
        }
        if ( ptr == musicalChairs) {
            StdOut.print(musicalChairs.getStudent().print() + " - POINTS TO FRONT");
        }
        StdOut.println();
    }

    /**
     * Prints the state of the classroom; can use this method to debug.
     * DO NOT edit.
     */
    public void printClassroom() {
        printStudentsInLine();
        printSeatedStudents();
        printMusicalChairs();
    }

    /**
     * Used to get and set objects.
     * DO NOT edit.
     */

    public SNode getStudentsInLine() { return studentsInLine; }
    public void setStudentsInLine(SNode l) { studentsInLine = l; }

    public SNode getMusicalChairs() { return musicalChairs; }
    public void setMusicalChairs(SNode m) { musicalChairs = m; }

    public boolean[][] getSeatingAvailability() { return seatingAvailability; }
    public void setSeatingAvailability(boolean[][] a) { seatingAvailability = a; }

    public Student[][] getStudentsSitting() { return studentsSitting; }
    public void setStudentsSitting(Student[][] s) { studentsSitting = s; }

}
